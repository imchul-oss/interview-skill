...
직무 역량(N개, 직무별 가변) + MVC 5가치 외 추가 자유 영역을 생성하지 않는다.
...
표준 질문 자체는 immutable이다. 본 절은 **렌더 시 부착하는 메타**(기법 라벨·관찰 포인트·실기 제안)의 규칙이며 질문 텍스트를 바꾸지 않는다. 기법 카탈로그의 정의는 companion `structured-interview/references/question_generation.md §6`을 따른다.
...
## 9. 시인성·인쇄 친화 작성 규칙 (v0.1.5, Critical)
...
7. 각 질문/가치 카드 사이 `---` 가로 구분선 삽입 (프리뷰에서 깔끔하게 분리). A4 강제 페이지 분리가 필요하면 PDF 변환 시 처리한다 (§9.3).

### 9.2 VS Code Preview (`Ctrl+Shift+V`) 친화
