...
6. **결정성** (v0.1.4): MVC 5가치 선별 금지, 직무 역량(N개, 직무별 가변)+5가치 외 자유 영역 신설 금지, 검증 우선순위 라벨 금지, 시간 배분은 `priority_algorithm.md §0` 결정적 알고리즘 사용
...
| 직무 역량(N개) + MVC 5가치 외 추가 자유 영역 신설 | 평가 기준 부동 (예: "AI 도메인 적응력" 같은 임의 추가) |
...
세부 규칙은 `references/output_personalized.md §9` 참조.
...
- ❌ 플러그인 외부에 격리된 과거 examples 시범 산출물 참조 (격리됨)
...
- **레벨별 BARS DB 마이그레이션**: 스킬로직·렌더·폴백은 v0.2.0 완료. 실제 `bars_anchors_by_level` 스키마 적용 + 기존 level='all' 행 분리는 시니어 패널 검토 후 별도 운영 작업 (본 플러그인 외부에서 관리)
